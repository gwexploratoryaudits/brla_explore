 function [n_Many, kmin_Many] = B2BRAVOkminMany(marginVector, alphaVector)
    ...
    %                       kmin(1) <= n(1). Last value of array n is 6*ASN 
    %                       for that margin and alpha.  
    %	kmin_Many:          each element of this list is a 1-D array kmin 
    %                       output by B2BRAVOkmin: 
    %                       kmin(j) = ceiling(kmslope*n(j) + kmintercept)
    %                       computed for the corresponding arrays n 
    %                       of n_Many. This function does not output 
    %                       kmslope or kmintercept. 
    % ----------
    ...
    for i=1:num_margin
        for s=1:num_alpha
            [kmslope, kmintercept, n_Many{i,s}, kmin_Many{i,s}] = B2BRAVOkmin(marginVector(i), alphaVector(s));
        end
    end
 end