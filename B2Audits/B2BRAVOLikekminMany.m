function [n_Many, kmin_Many] = B2BRAVOLikekminMany(marginVector, alphaVector, NVector)
    ...
    %                       arrays)
    %	n_Many:         each element of this list is a 1-D array n output 
    %                       by B2BRAVOLikekmin. It begins at the smallest 
    %                       sample size n(1) for which kmin(1) <= n(1) 
    %                       and the sample size gives a large enough 
    %                       likelihood ratio. The last value of n is the 
    %                       corresponding value of N. 
    %	kmin_many:      each element of this list is a 1-D array kmin from
    %                       B2BRAVOLikekmin; kmin(j) is the minimum 
    ...
    for i=1:num_margin
        for s=1:num_alpha
             for t=1:num_N
                [n_Many{i,s,t}, kmin_Many{i,s,t}, ratio] = B2BRAVOLikekmin(marginVector(i), alphaVector(s), NVector(t));
                % Observe that we do not care about the LLR here; the 
                % LLR is output by B2BRAVOLikekmin as ratio. 
             end
        end
    end
end
