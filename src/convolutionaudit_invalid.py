...
class Convolution_Audit:
    ...
    def __init__(self, N, round_schedule, risk_schedule, invalid_schedule):
        ...
        # The invalid schedule is a list of integers where the invalid_schedule[i] is the number
        # of invalid votes found in the (i + 1)'th round (not cumulative). Clearly in an actual
        # audit this will be gradually populated.
        self.invalid_schedule = invalid_schedule
    ...
    def conduct_audit(self):
        ...
        self.invalid_bookkeeping(round_index)
    ...
    def invalid_bookkeeping(self, round_index):
        # However many invalid votes we found this round we subtract from N and the round schedule.
        # There are likely more invalid votes, but this is conservative.
        invalid_votes = self.invalid_schedule[round_index]

        self.N -= invalid_votes
        self.half_N = int(self.N / 2)

        for round_size in range(round_index, self.number_of_rounds):
            self.round_schedule[round_size] -= invalid_votes
        self.half_rounds = [int(round_size / 2) for round_size in self.round_schedule]
