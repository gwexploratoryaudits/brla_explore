...
To validate our mathematical approach and code for B2 audits we have computed the values of Table 1 in the *BRAVO* paper. See: B2Audits/Tables/BRAVO Table I.pdf for the first five rows and Tables/BRAVO Table II.pdf for the next five rows. The largest fractional difference is smaller than 0.5\%, and occurs in estimating the expected number of ballots in simulations of audits for an election with a 1\% margin.  This difference could reflect an inaccuracy in our approach or the finiteness of the number of simulations used to generate the BRAVO table. 
