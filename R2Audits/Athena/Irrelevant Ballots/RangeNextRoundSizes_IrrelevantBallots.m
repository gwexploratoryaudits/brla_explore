function [next_rounds_max, next_rounds_min, n, kmin, Stopping] = ...
    ...
    %   RangeNextRoundSizes_IrrelevantBallots(margin, alpha, delta, StopSched_prev, ...
    %   RiskSched_prev, CurrentTierStop, CurrentTierRisk, n_last, ... 
    %   k_last, percentiles, max_draws, audit_method, irrelevant_fraction)
    %
    % Computes range of next round sizes for given percentiles.
    % Same as RangeNextRoundSizes.m but using new irrelevant ballot
    % approach.
    %
    % ---------------------------Inputs------------------------
    %
