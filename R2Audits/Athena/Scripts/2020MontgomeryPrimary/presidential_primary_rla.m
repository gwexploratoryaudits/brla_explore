...
%
    ...
    n_actual = zeros(size(candidates)); % current cumulative relevant sample size
    
    % Read sample from sampled.m
    sampled;
    
    % Bad manual fix I here: no votes in sample for candidate 1, who is 
    % ignored in the sample so we stick in a zero column here to make 
    % the sample consistent with the candidates. 
    ...
    % write-ins from the candidates.
    new_sample_2 = new_sample(:, [1:3 5:11]); 
    new_candidates = candidates(1:size(candidates,1)-1);
    new_losers = losers(1:size(losers,2)-1);
    cumulative_sample = zeros(size(new_sample_2));
   
    % Compute number of ballots for each candidate after each draw
    for i=1:size(new_sample_2,1)
        cumulative_sample(i, 1:size(new_sample_2,2)) = sum(new_sample_2(1:i,1:size(new_sample_2,2)));
    end
