...
% using sample data from sampled_ccr.m
...
%
    ...
    p_max_minerva(i) = min(1, max(p_minerva(i, :)));
    p_max_eor(i) = min(1, max(p_eor(i, :)));
    p_max_sb(i) = min(1, max(p_sb(i, :)));
...
% Label axes
xlabel('Number of ballots drawn', 'FontSize', 18)
ylabel('Computed p-value', 'FontSize', 18)
title('County Commissioner FTC 1-2-2021, Republican', 'FontSize', 18) 
