...
if NumberRounds > 1
    for j=2:NumberRounds
        pvalues_Arlo(j) = p_value(margin, n_Arlo(1:j-1), kmin_Arlo(1:j-1), k_all(1:j-1), n_Arlo(j), k_all(j), 'Arlo');
        pvalues_Athena(j) = p_value(margin, n_Athena(1:j-1), kmin_Athena(1:j-1), k_all(1:j-1), n_Athena(j), k_all(j), 'Athena');
        pvalues_Minerva(j) = p_value(margin, n_Minerva(1:j-1), kmin_Minerva(1:j-1), k_all(1:j-1), n_Minerva(j), k_all(j), 'Minerva');
    end
end
