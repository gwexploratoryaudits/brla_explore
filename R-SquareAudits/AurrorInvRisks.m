function [kmin, ActualRiskSched, ActualRiskValue, ActualStopSched, ActualStopProb, ExpectedBallots] = AurrorInvRisks(margin, N, n, RiskSched, StopSched, audit_type)
    %
    % [kmin, ActualRiskSched, ActualRiskValue, ActualStopSched, ActualStopProb, ExpectedBallots] = AurrorInvRisks(margin, N, n, RiskSched, StopSched, audit_type)
    % This function returns:
    %       kmin values    
    % for an Aurror audit defined by a round schedule n, corresponding 
    % risk and stopping prob. schedules RiskSched and StopSched, applied 
    % to an election defined by margin and total size N.    
    %----------
    % Input Values: 
    ...
    %   StopSched:      row vector of same size as n 
    ...
    %	ActualStopSched:	array of individual risk values corresponding 
    %                           to kmin.                      
    %   ActualStopValue:	the actual risk computed as the sum of all 
    %                           values of the actual risk sched.
    ...
    ActualStopSched = zeros(1,NumberRounds);
